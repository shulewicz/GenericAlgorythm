...
namespace MomAndDadAlgorytm
{
    public class DAP
    {
        ...
        public DAP (Network network)
        {
        }

        public void bruteForce (Network Network)
        {
            solution = new List<List<int>>();
            lambdasOnLink = new List<int>();

            for (int i = 0; i < Network.Link.Count(); i++)
            {
                lambdasOnLink.Add(0);
            }
            ...
            for (int i = 0; i<Network.Demand.Count(); i++)
            {
                //Console.WriteLine("Demand:" + (i+1));
                //Console.WriteLine("H: " + Network.Demand[i].demandVolume);
        ...
        private bool solutionCheck (Network Network)
        {
            ...
            for (int i = 0; i<lambdasOnLink.Count(); i++)
            {
                ...
                //Console.WriteLine("Number of lambdas: " + lambdasOnLink[i]);
                //Console.WriteLine("Max number lambdas on link: " + (Network.Link[i].numberOfLambdas * Network.Link[i].numberOfFibres));
                //Console.WriteLine("");
